...
class CalendarImplem(
    ...
    override fun createCalendar(
        ...
        if (!permissionHandler.requestWritePermissionSync()) {
            callback(
                Result.failure(
                    FlutterError(
                        code = "ACCESS_REFUSED",
    ...
    override fun retrieveCalendars(
        ...
        if (!permissionHandler.requestReadPermissionSync()) {
            callback(
                Result.failure(
                    FlutterError(
                        code = "ACCESS_REFUSED",
        ...
        try {
            ...
            cursor?.use { c ->
                ...
                while (c.moveToNext()) {
                    ...
                    val attendees = retrieveAttendees(id)
                    val reminders = retrieveReminders(id)
    ...
    override fun createReminder(reminder: Long, eventId: String, callback: (Result<Event>) -> Unit) {
        ...
        try {
            ...
            val event = retrieveEvent(eventId)
            callback(Result.success(event))
    ...
    private fun retrieveEvent(
        ...
        return event ?: throw FlutterError(
            code = "NOT_FOUND",
            message = "Failed to retrieve event"
        )
    }

    private fun retrieveReminders(eventId: String): List<Long> {
    ...
    private fun retrieveAttendees(eventId: String): List<Attendee> {
        val projection = arrayOf(
